function ConvertGoogleDocToCleanHtml() {
  var b = DocumentApp.getActiveDocument().getBody();
  var body = b;
  body.replaceText(">", "&gt;");
  body.replaceText("<", "&lt;");
  ...
  for (var i = 0; i < numChildren; i++) {
    ...
    output.push(processItem(child, listLevel, images));
  }
...
function processItem(item, listLevel, images) {
  var output = [];
  var prefix = "", suffix = "";
  ...
  else if (item.getType() == DocumentApp.ElementType.INLINE_IMAGE)
  {
    ...
    // First list item
    if (listItem.getNestingLevel() > listLevel[0]) {
      listLevel[0] = listItem.getNestingLevel();
      listLevel[1] = listItem.getNestingLevel();
      // Bullet list (<ul>):
    ...
    // Last list item
    if (item.isAtDocumentEnd() || (item.getNextSibling() && (item.getNextSibling().getType() != DocumentApp.ElementType.LIST_ITEM)) || listItem.getNestingLevel() < listLevel[0]) {
      if (gt === DocumentApp.GlyphType.BULLET || gt === DocumentApp.GlyphType.HOLLOW_BULLET || gt === DocumentApp.GlyphType.SQUARE_BULLET) {
